...
| # | 需求 | 阶段 | 状态 |
|---|------|------|------|
| 1 | 生成 PPT 能力（读 PDF → 高级感 PPT） | P1 | ⬜ |
| 2 | 桌面报告：为什么 CLI 能出 PPT、应用不行 | P1 | ⬜ |
| 3 | 对话产物优先存到该对话的工作文件夹 | P2 | ⬜ |
| 4 | 「参考资料」改文件夹：按时间排列本对话产物，点开右栏速览 | P2 | ⬜ |
| 5 | 历史对话搜索把过往输出文件也算入 | P2 | ⬜ |
| 6 | 沙箱替换为 CubeSandbox（替掉 Docker 方案） | P3 | ⚠️ |
| 7 | 对话进程多开（像豆包 / WorkBuddy） | P3 | ⬜ |
| 8 | 沙箱「工位」可视化（约 9 个）+ 古风数字人 | P4 | ⬜ |
| 9 | 任务完成、用户未看 → 右侧对话加墨蓝色提醒点 | P5 | ⬜ |
| 10 | 新模块布局规划，避免带崩其他板块 | 贯穿 | 🔧 |
| 11 | 修改 PRD | 贯穿 | 🔧 |
| 12 | 规划任务 | P0 | ✅ |
